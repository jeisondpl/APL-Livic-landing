...
export default function HeroLanding() {
  ...
  const [titlePart1, titlePart2] = LANDING_CONTENT.hero.tagline.includes(':') ? LANDING_CONTENT.hero.tagline.split(':').map((s) => s.trim()) : [LANDING_CONTENT.hero.tagline, '']

  return (
    <section className='w-full min-h-screen px-4 md:px-8 xl:px-14 py-12 flex items-center'>
      <div className='relative w-full max-w-6xl mx-auto rounded-3xl shadow-2xl overflow-hidden' style={{ minHeight: '600px' }}>
        ...
        <div className='relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center px-6 md:px-12 py-12 lg:py-16'>
          ...
          <motion.div variants={containerVariants} initial='hidden' animate='visible' className='space-y-5'>
            ...
            <motion.div variants={itemVariants}>
              <img src='/logo-livic-white.png' alt='LIVIC' className='h-12 md:h-14 w-auto mb-5' />
            </motion.div>
              ...
              <h1 className='text-3xl md:text-4xl lg:text-5xl font-black text-white leading-tight'>
                {titlePart2 && (
                  <>
                    ...
                    <br />
                    <span className='inline-block bg-gradient-to-r from-livic-pink to-livic-purple bg-clip-text text-transparent'>{titlePart2}</span>
                  </>
                )}
              </h1>
            </motion.div>
            ...
            {/* Subtítulo */}
            <motion.div variants={itemVariants}>
              <p className='text-base md:text-lg text-white/90 leading-relaxed'>{LANDING_CONTENT.hero.subtitle}</p>
            </motion.div>

            {/* Lista de bullets con checkmarks */}
            <ul className='space-y-3'>
              {HERO_BULLETS.map((item) => (
                <motion.li key={item} variants={itemVariants} className='flex items-center gap-3 text-white/90'>
          ...
          >
            {/* Título del panel */}
            <h3 className='text-xl font-bold text-gray-900 mb-2'>Conoce nuestros servicios</h3>
            <p className='text-sm text-gray-500 mb-6'>Descubre cómo LIVIC puede ayudarte</p>
            ...
            {/* Nota de confianza */}
            <p className='text-xs text-gray-400 mt-5 text-center'>✓ Respondemos en menos de 24 horas</p>
        ...
        <motion.div
          ...
          onClick={handleScrollToServicios}
        >
          <span className='text-white text-xs uppercase tracking-wider'>Descubre más</span>
